...
test("Should resolve a promise to '21,34,43,57,'Anukriti'' ", function () {
  return expect(
    promisifyFs("./promisify-file-read/testFile.txt")
  ).resolves.toBe('21,34,43,57,"Anukriti"');
  //done();
});
...
test("Unit testing for resolved", function (done) {
  jest
    .spyOn(fs, "readFile")
    .mockImplementation(function (file, option, callback) {
...
test("Unit testing for resolved using .resolves", function () {
  jest
    .spyOn(fs, "readFile")
    .mockImplementation(function (file, option, callback) {
      callback(null, '21,34,43,57,"Anukriti"');
...
test("Unit testing for rejected", function () {
  jest
    .spyOn(fs, "readFile")
    .mockImplementation(function (file, option, callback) {
      callback(new Error("An error occured"), null);
    });
  return expect(promisifyFs("randomFile")).rejects.toEqual(new Error("An error occured"));
});
